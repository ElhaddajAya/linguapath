...
// THEMES reste en français : ce sont des données de contenu (catégories de scénarios
// stockées en base), pas du texte d'interface — même logique que Scenario.jsx
const THEMES = [
  "Restaurant",
  "Voyage",
  "Entretien",
...
export default function AdminScenarios() {
  ...
  const chargerScenarios = async () => {
    ...
    try {
      ...
      showToast(t("adminScenarios.errorLoad"), "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const sauvegarder = async () => {
    if (
      ...
      showToast(t("adminScenarios.fillRequired"), "error");
      return;
    }
    setSaving(true);
    try {
      if (editTarget) {
        ...
        showToast(t("adminScenarios.editSuccess"));
      } else {
        const res = await api.post("/admin/scenarios", form);
        setScenarios((prev) => [res.data.scenario, ...prev]);
        showToast(t("adminScenarios.createSuccess"));
      }
      fermerModal();
    } catch (err) {
      showToast(
        err.response?.data?.message || t("adminScenarios.errorSave"),
        "error",
      );
    } finally {
      setSaving(false);
  ...
  const confirmerSuppression = async () => {
    ...
    try {
      ...
      showToast(t("adminScenarios.deleteSuccess"));
    } catch {
      showToast(t("adminScenarios.errorDelete"), "error");
    } finally {
      setDeleting(false);
  ...
  const filtres = scenarios.filter((sc) => {
    const matchRecherche =
      !search || sc.titre?.toLowerCase().includes(search.toLowerCase());
        ...
        <div className='flex items-start sm:items-center justify-between mb-6 sm:mb-8 flex-wrap gap-4'>
          <div>
            ...
            >
              ...
              />
              <span>{t("adminScenarios.users")}</span>
              <Users
                size={11}
                className='ml-0.5'
              />
            ...
            <p className='text-warm-500 text-xs sm:text-sm mt-1'>
              {scenarios.length}{" "}
              {scenarios.length !== 1
                ? t("adminScenarios.availables")
                : t("adminScenarios.available")}
            </p>
          </div>
          ...
          >
            ...
            <span className='hidden sm:inline'>
              {t("adminScenarios.newScenario")}
            </span>
            <span className='sm:hidden'>{t("adminScenarios.new")}</span>
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className='bg-white rounded-2xl border border-warm-200 shadow-soft p-10 sm:p-12 text-center'>
            ...
            <p className='text-warm-600 font-medium mb-2'>
              {t("adminScenarios.noScenario")}
            </p>
            <p className='text-warm-400 text-sm mb-5'>
              {t("adminScenarios.noScenarioSub")}
            </p>
            <button
              onClick={ouvrirCreation}
            ...
            >
              <Plus size={15} /> {t("adminScenarios.createFirst")}
            </button>
          </div>
        ) : filtres.length === 0 ? (
          <div className='bg-white rounded-2xl border border-warm-200 shadow-soft p-10 text-center'>
            <Search
              size={36}
              className='text-warm-300 mx-auto mb-3'
            />
            <p className='text-warm-600 font-medium'>
              {t("adminScenarios.noMatch")}
            </p>
            <button
              onClick={() => {
                setSearch("");
            ...
            {filtres.map((sc) => (
              ...
              >
                ...
                <div className='flex flex-wrap gap-1.5 sm:gap-2'>
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${NIVEAU_COLORS[sc.niveauMin]}`}
                  >
                    {sc.niveauMin}
                  </span>
                  <span className='text-xs text-warm-400 self-center'>→</span>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${NIVEAU_COLORS[sc.niveauMax]}`}
                  >
                    {sc.niveauMax}
                  </span>
                </div>
        ...
        >
          ...
          >
            ...
            <div className='flex gap-3'>
              <div className='flex flex-col gap-1.5 flex-1'>
                ...
                <select
                  ...
                  className='px-3 py-2.5 rounded-xl border border-warm-200 text-sm text-warm-700
                    bg-warm-50 focus:outline-none focus:border-orange-300'
                >
                  <option value=''>{t("adminScenarios.choose")}</option>
                  {LANGUES.map((l) => (
                    <option
                      key={l}
                      value={l}
              ...
              <div className='flex flex-col gap-1.5 flex-1'>
                <label className='text-xs font-medium text-warm-600'>
                  {t("adminScenarios.themeRequired")}
                </label>
                <select
                  value={form.theme}
                  onChange={(e) =>
            ...
            <div className='flex flex-col gap-1.5'>
              <label className='text-xs font-medium text-warm-600'>
                {t("adminScenarios.descriptionRequired")}
              </label>
              <textarea
            ...
            <div className='flex flex-col gap-1.5'>
              ...
              <p className='text-xs text-warm-400'>
                {t("adminScenarios.systemPromptHint")}
              </p>
            </div>
        ...
        >
          ...
          >
            <div className='text-center'>
              <div
                className='w-14 h-14 rounded-2xl bg-red-50 border border-red-100
              ...
              <h3 className='font-semibold text-warm-900 text-lg mb-2'>
                {t("adminScenarios.confirmDelete")}
              </h3>
              <p className='text-sm text-warm-500 leading-relaxed'>
                {t("adminScenarios.confirmDeleteSub")}
              </p>
            </div>
            <div className='flex gap-3'>
              <button
