...
import { ArrowLeft, Settings, Pencil, Check, X, Eye, EyeOff } from "lucide-react";
...
export default function Profile() {
  ...
  const sauvegarder = async () => {
    ...
    if (changePwd) {
      ...
      if (newPwd.length < 6) {
        showToast("Le nouveau mot de passe doit contenir au moins 6 caractères.", "error"); return;
      }
  ...
  const renderAvatar = (emoji, size = "lg") => {
    const dim = size === "lg" ? "w-20 h-20 text-4xl" : "w-10 h-10 text-xl";
    if (emoji) {
      return (
        <div className={`${dim} rounded-full flex items-center justify-center bg-warm-100 border-2 border-warm-200`}>
          {emoji}
        </div>
      );
    }
    return (
      <div className={`${dim} rounded-full flex items-center justify-center text-white font-bold`}
  ...
  const inputCls = "w-full px-4 py-2.5 rounded-xl border border-warm-200 text-sm text-warm-900 bg-warm-50 focus:outline-none focus:border-orange-300 placeholder:text-warm-300";
  ...
  return (
    <div className="min-h-screen bg-warm-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-10 py-8 sm:py-10">
        <h1 className="text-2xl font-semibold text-warm-900 mb-8">Mon profil</h1>
            ...
            <div className="bg-white rounded-2xl border border-warm-200 shadow-soft p-6">
              <div className="flex flex-col items-center text-center gap-3 pb-6 border-b border-warm-200">
                ...
                {editing ? (
                  ...
                  <div>
                    <h2 className="font-semibold text-warm-900 text-lg">{user.nom}</h2>
              ...
              {editing && (
                <div className="py-4 border-b border-warm-200">
                  <p className="text-xs font-semibold text-warm-400 uppercase tracking-widest mb-3">Choisir un avatar</p>
                  <div className="grid grid-cols-6 gap-2">
                    <button onClick={() => setAvatarEdit("")}
                    ...
                    {AVATARS.map((emoji) => (
                      <button key={emoji} onClick={() => setAvatarEdit(emoji)}
                ...
                {editing ? (
                  <>
                    <button onClick={sauvegarder} disabled={saving}
                      ...
                      style={{ background: "linear-gradient(135deg, #F59E0B, #EA580C)" }}>
                      <Check size={15} />{saving ? "Sauvegarde..." : "Sauvegarder"}
                    </button>
            ...
            {editing && (
              <div className="bg-white rounded-2xl border border-warm-200 shadow-soft p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => { setChangePwd(!changePwd); setCurrentPwd(""); setNewPwd(""); setConfirmPwd(""); }}
                ...
                {changePwd ? (
                  <div className="flex flex-col gap-3">
                    {/* Mot de passe actuel */}
                    <div>
                      <label className="text-xs font-medium text-warm-600 mb-1 block">Mot de passe actuel</label>
                    ...
                    <div>
                      <label className="text-xs font-medium text-warm-600 mb-1 block">Nouveau mot de passe</label>
                      <div className="relative">
                        <input type={showNew ? "text" : "password"} value={newPwd}
                          onChange={(e) => setNewPwd(e.target.value)}
                          placeholder="Min. 6 caractères" className={inputCls + " pr-10"} />
                    ...
                    {/* Confirmer */}
                    <div>
                      <label className="text-xs font-medium text-warm-600 mb-1 block">Confirmer le mot de passe</label>
                      <div className="relative">
                      ...
                      {/* Indicateur match */}
                      {confirmPwd && (
                        <p className={`text-xs mt-1 ${newPwd === confirmPwd ? "text-green-600" : "text-red-500"}`}>
                          {newPwd === confirmPwd ? "✓ Les mots de passe correspondent" : "✕ Ne correspondent pas"}
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-warm-200 shadow-soft p-6">
              {user.role === "admin" ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-warm-500 text-sm mb-6">Gérez les utilisateurs et les scénarios depuis le panneau admin.</p>
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    ...
                    <button onClick={() => navigate("/admin/scenarios")}
                  ...
                  {user.langues?.length > 0 ? (
                    <div className="flex flex-col gap-3">
                      {user.langues.map((l) => (
                        <div key={l.langue}
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-xl">{LANGUE_EMOJI[l.langue] || "🌍"}</span>
                            <span className="font-medium text-warm-900">{l.langue}</span>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className={`text-xs font-semibold px-3 py-1.5 rounded-full ${NIVEAU_STYLE[l.niveau]}`}>
                              {l.niveau}
                            </span>
                            <button onClick={() => navigate(`/scenarios?langue=${l.langue}`)}
                      ...
                      <button onClick={() => navigate("/quiz")}
                        ...
                        <span className="text-lg">+</span> Ajouter une nouvelle langue
                      </button>
                    </div>
                  ) : (
                    <div className="text-center py-12">
                      <p className="text-5xl mb-4">🌍</p>
                      <p className="text-warm-500 text-sm mb-5">Aucune langue en apprentissage pour l'instant.</p>
                      <button onClick={() => navigate("/quiz")}
                        className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-opacity"
                        style={{ background: "linear-gradient(135deg, #F59E0B, #EA580C)" }}>
                        Choisir une langue
